<?php
class Data_DataAccess {
	private $db_conf_file = "Data/dbconf.php";
	private $db_exceptions = "Data/DataAccess/";
	...
	protected function __construct(){
		...
		// Eccezioni
		require_once $_SERVER['DOCUMENT_ROOT']."Data/DataAccess/ConnException.php";
		require_once $_SERVER['DOCUMENT_ROOT']."Data/DataAccess/SelectException.php";
		...
		if($this->connessione === FALSE){
			...
			throw new Data_DataAccess_ConnException('DB connection error.');
		}
		...
		if(mysql_select_db($db_name, $this->connessione)){
			...
			throw new Data_DataAccess_SelectException('DB selection error.');
		}
	}
	...
	/**
	 * Restituisce un array associativo con i risultati della query
	...
	/**
	* Restituisce un oggetto json contenente i risultati della query
	* @return json obj
	* @example	Esternamente all'oggetto Data_access:
	* 			$query = "SELECT * FROM carpooler";
	*			$esito = $dao->invia_query($query);
	*			print($dao->get_json_obj($esito));
	*/
	public function get_json_obj(){
		while($row = mysql_fetch_assoc($this->query_result)) $output[] = $row;
		//var_dump($output);		
		krumo($output);
	 ...
	 * dˆ in uscita un array che descrive i campi in comune
	...
	$righe = null) {
		...
		foreach ($fields_name_array as $field) {
			if(key_exists($field, $conn)){
				//il campo � uno dei connettori
				$query .= $conn[$field];
			}else{
				//il campo non � un connettore
				$query .= $field;
			}
			$query .= ", ";
		}
		...
		if(is_array($and_where) && $and_where !== array()){
			foreach($and_where as $field1 => $field2){
				if (is_null($field2) || $field2 == '') {
					...
					// se � una stringa aggiungo gli apici
					$query .= $field1." = '".$field2."'";
				}
				$query .= " AND ";
			}
